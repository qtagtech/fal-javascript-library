/*!
  * FreeAppsListing JavaScript tracking library
  * Copyright (c) 2013 Amplificious Ltd
  * https://www.amplificious.com
  * MIT license
  */
...
// Main library object
FreeAppsListing.Library = function(queue, options) {
    this._version = LIB_VERSION;
    this[CALLBACK_STORE] = {};
    this.options = options || {};
    this.queue = new FreeAppsListing.Queue(queue, {
        execute: _.bind(this._execute, this)
    });
};

_.extend(FreeAppsListing.Library.prototype, {
    ...
    trackClick: function() {
        this._trackDOMEvent(FreeAppsListing.ClickTracker, arguments);
    },

    trackSubmit: function() {
        this._trackDOMEvent(FreeAppsListing.SubmitTracker, arguments);
    },
    ...
    _trackDOMEvent: function(DOMEventTracker, trackArgs) {
        var elements = FreeAppsListing.utils.getElements(trackArgs[0]);
    ...
    _getMetadata: function() {
        return {
            $page_info: FreeAppsListing.utils.pageInfo(),
...
// Main call queue
FreeAppsListing.Queue = function(queue, options) {
    this.options = options || {};
    this.executeAll(queue);
};

_.extend(FreeAppsListing.Queue.prototype, {
...
FreeAppsListing.Cookie = function(options) {
    this.options = options || {};
    this.props = {};
    this.name = "fal_" + _.cookie.utils.escape(options.name);
    this.load();
    this.setOnce({
...
_.extend(FreeAppsListing.GetRequest.prototype, FreeAppsListing.Request, {
    ...
    getRequestType: function() {
        // Call "super"
        var type = FreeAppsListing.Request.getRequestType();
        if (type) return type;
    ...
    _storeCallback: function(opts) {
        ...
        return FAL_QUEUE+"."+CALLBACK_STORE+'["'+randomToken+'"]';
    }
...
FreeAppsListing.PostRequest = function(options) {
    this.options = _.extend({}, {
        method: "POST"
    }, options);
    this.send();
...
FreeAppsListing.DOMEventTracker = {
...
FreeAppsListing.ClickTracker = function(options) {
    this.options = options || {};
...
_.extend(FreeAppsListing.ClickTracker.prototype, FreeAppsListing.DOMEventTracker, {
...
FreeAppsListing.SubmitTracker = function(options) {
    this.options = options || {};
    this.initialize.apply(this, arguments);
};

_.extend(FreeAppsListing.SubmitTracker.prototype, FreeAppsListing.DOMEventTracker, {
...
FreeAppsListing.utils = {

    pageInfo: function() {
        return FreeAppsListing.utils.truncateData(
            FreeAppsListing.utils.stripEmpty({
    ...
    truncateData: function(obj, length) {
        length = length || 255;
        var result, truncate = FreeAppsListing.utils.truncateData;
        if (_.isObject(obj)) {
            result = {};
    ...
    getElements: function(selector) {
        var results = [],
            getRecursive = FreeAppsListing.utils.getElements,
